...
public class CompensationTransactionService {
	...
	public PaymentCancelResponse compensateCancelCompleteFailure(String orderId, Throwable cause) {
		...
		log.error("[결제-Comp] 취소 완료로 수정 실패: 상태 오류, orderId={}, status={}", orderId, payment.getPayStatus());
		throw new IllegalStateException("취소 완료로 수정 실패 - 잘못된 상태: " + payment.getPayStatus());
	}

	/** 빌링키 발급 저장 실패 (보상 없음, 예외만) */
	public void compensateIssueKeyFailure(String orderId, String billingKey, Throwable cause) {
		log.error("[결제-Comp] 빌링키 저장 실패: orderId={}, billingKey={}, cause={}", orderId, billingKey, cause.toString());
		throw new IllegalStateException("빌링키 저장 재시도 실패", cause);
	}
